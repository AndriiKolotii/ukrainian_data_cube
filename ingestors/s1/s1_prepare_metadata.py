...
def prep_dataset(fields, path):
    aos = datetime.strptime(path.stem[17:32], '%Y%m%dT%H%M%S')
    los = datetime.strptime(path.stem[33:48], '%Y%m%dT%H%M%S')
    cos = aos + (los - aos) / 2
    start_time = aos
    end_time = los
